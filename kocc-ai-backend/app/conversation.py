...
class ConversationContext:
    ...
    pending_operational_namespace: str | None = None
    ...
    def from_payload(cls, value: Any) -> "ConversationContext":
        ...
        if pending_intent not in {
            ...
            pending_intent = None
        pending_parameter = value.get("pending_operational_parameter")
        if pending_parameter not in {"cluster", "namespace"}:
            pending_parameter = None
        pending_scope = value.get("pending_operational_scope")
        if pending_scope not in {"inventory", "namespace"}:
            pending_scope = None
        pending_namespace = _safe_name(value.get("pending_operational_namespace"))
        valid_pending = pending_intent is not None and (
            pending_parameter == "namespace" and safe_pending_cluster is not None
            and pending_scope == "namespace"
            or pending_intent == "egressip_lookup"
            and pending_parameter == "cluster"
            and pending_scope in {"inventory", "namespace"}
        )
        ...
        return cls(
            ...
            pending_operational_intent=pending_intent,
            ...
            pending_operational_namespace=pending_namespace,
        )
    ...
    def public_dict(self) -> dict[str, Any]:
        ...
        if self.pending_operational_namespace is not None:
            payload["pending_operational_namespace"] = self.pending_operational_namespace
        return payload
    ...
    def without_pending_suggestion(self) -> "ConversationContext":
        return ConversationContext(
            ...
            pending_operational_namespace=self.pending_operational_namespace,
        )
    ...
    def with_active_clusters(self, cluster_ids: tuple[str, ...]) -> "ConversationContext":
        ...
        return ConversationContext(
            ...
            pending_operational_namespace=(
                self.pending_operational_namespace
                if self.pending_operational_cluster_id in {None, *cluster_ids}
                else None
            ),
        )
    ...
    def with_operational_focus(
        ...
        return ConversationContext(
            ...
            pending_operational_namespace=None,
        )

    def with_pending_operational(
        self, intent: str, cluster_id: str | None,
        *, parameter: str = "namespace", scope: str = "namespace",
        namespace: str | None = None,
    ) -> "ConversationContext":
        if intent not in {"inspect_pods", "inspect_events", "egressip_lookup"}:
            return self
        return ConversationContext(
            active_cluster_ids=(cluster_id,) if cluster_id else (),
            last_resource_kind=self.last_resource_kind,
            last_namespace=self.last_namespace,
            last_query_operation=self.last_query_operation,
            last_operation=self.last_operation,
            ...
            pending_operational_parameter=parameter,
            pending_operational_scope=scope,
            pending_operational_namespace=_safe_name(namespace),
        )
